...
parser.add_argument('interp_file' , type = str, help = 'Path to file containing interpolated values. Need to be .npz file.')
...
#load in interp arrays
interp_varbs = np.load(args.interp_file, allow_pickle=True)
interp_names = ['names','int_time', 'int_logt', 'int_logg', 'int_mass', 'tmap_valid']
for k in range(len(interp_names)):
    locals()[interp_names[k]] = interp_varbs['arr_'+str(k)]
...
for i in tqdm (range(len(data_files)), #(len(data_files)), 
    ...
    if tmap_valid[i] == True:
        ax[0].scatter(int_time[i], int_logt[i], color = 'forestgreen', label = 'time = '+str(int_time[i])+';\nlog(T) = '+str(int_logt[i]))
    else:
        ax[0].axvline(x=int_time[i], linestyle=':', color = 'forestgreen', label = 'Invalid interpolation')
    ...
    if tmap_valid[i] == True:
        ax[1].scatter(int_time[i], int_logg[i], color = 'forestgreen', label = 'time = '+str(int_time[i])+';\nlog(g) = '+str(int_logg[i]))
    else:
        ax[1].axvline(x=int_time[i], linestyle=':', color = 'forestgreen', label = 'Invalid interpolation')
