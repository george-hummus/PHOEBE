        ...
        for k in incls:
            ...
            if b.run_checks().status != 'FAIL': #under the condition that the compute won't fail
                try:
                    ...
                    cycle_out = [MS_mass, period, incl,'ck2004', n_amp, oh_amp, ampDiff, is_eclipse]
                    ...
                    try: #try with the phoenix atmosphere model if ck2004 doesn't work
                        ...
                        ## Saving out results to list, so can be saved as csv file
                        cycle_out = [MS_mass, period, incl, 'phoenix', n_amp, oh_amp, ampDiff,is_eclipse]

                        succs.append(1)

                    except ValueError as error1:
                        print('phoenix error')
                        cycle_out = [MS_mass, period, incl, False, False, False, False, False]

                        fails.append(1)

            else:
                cycle_out = [MS_mass, period, incl,None, None, None, None, None]
                fails.append(1)
